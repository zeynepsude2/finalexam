	  ...
	  if (strlen((char*)buffer) > 0)
	    {
	  	  ...
	  	  {
	  		  sntByt = buffer[i];
	  		  //23-PA0 input, 31-PA6 output
	  		  for (int j = 0; j < 8; j++)
	  		  {
	  		      HAL_GPIO_WritePin(GPIOD, output1_Pin, sntByt & (1 << j) == 0 ? GPIO_PIN_RESET : GPIO_PIN_SET);
	  		      HAL_Delay(500);
	  		      int value= HAL_GPIO_ReadPin(GPIOD,input1_Pin);
	  		      char out[10]={0};
	  		      snprintf(out, 9, "%d/n", value);
